...
description: Generate interactive 3D zoning envelope viewers from zoning analysis reports. Use when the user asks to "visualize the zoning envelope" or see buildable massing in 3D, typically after /as:zoning-analysis-nyc or /as:zoning-analysis-austin. Requires a zoning analysis report as input.
allowed-tools:
  - Read
  - Write
  - Bash
...
**Requires a zoning analysis report** generated by `/as:zoning-analysis-nyc` or `/as:zoning-analysis-austin`. This skill is a renderer — it does not perform zoning calculations.
...
> No zoning analysis report found. Run the applicable zoning analysis skill first (`/as:zoning-analysis-nyc` or `/as:zoning-analysis-austin`), then come back with `/as:zoning-envelope`.
