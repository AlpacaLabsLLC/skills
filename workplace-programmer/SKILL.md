...
### Reports & Exports
When the user asks for a report, export, or download — or when a program is fully detailed:
- Write a `report.md` with the full program summary (area splits, seats, rooms, support)
- Write a `report.csv` with the line-item detail
- Both files go in the current directory
