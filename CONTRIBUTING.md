...
Repository maintainers can run the matching private workflow as `$release` on Codex or `/release` on Claude Code. The host-local cards live at [`.agents/skills/release`](./.agents/skills/release/SKILL.md) and [`.claude/skills/release`](./.claude/skills/release/SKILL.md); they are Studio Operations extensions, not installed public `/as:*` catalog skills.
